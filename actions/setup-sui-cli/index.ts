...
async function ensureSuiInstalled(version: string, installDir: string, suiBinPath: string): Promise<void> {
...
async function configureWallet(network: string, privateKey: string): Promise<void> {
...
async function main(): Promise<void> {
  ...
  if (!SUPPORTED_NETWORKS.has(network)) {
    throw new Error(`Unsupported network '${network}'. Supported values are: mainnet, testnet.`);
  }
  ...
  if (privateKey) {
    ...
    core.warning('private_key input is empty; installed Sui CLI only and skipped wallet configuration.');
  }
}
